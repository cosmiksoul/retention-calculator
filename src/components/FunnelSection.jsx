...
// The last step's count becomes the "acquired pool" — the people whose
// retention we model and to whom ARPU is per-period.
//
...
/**
 * @param {{
 *   funnel: Array<{id?:string, label:string, conversionPct:number}>,
 *   onChange: (next: Array<{id:string, label:string, conversionPct:number}>) => void,
 *   defaultOpen?: boolean,
 * }} props
 */
export default function FunnelSection({ funnel, onChange, defaultOpen }) {
  ...
  const add = () => {
    ...
    onChange([
      ...rows,
      { id: newPointId(), label: `Step ${rows.length + 1}`, conversionPct: 50 },
    ])
  }
                ...
                <div key={r.id}>
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => remove(r.id)}
                      className="ml-auto shrink-0 text-base leading-none text-fg-faint hover:text-red-400"
